...
基于 **SM-2** 的 Obsidian 记忆卡片抽查与复习调度 skill。它负责把卡片扫入题库、挑出到期卡片、维护 pending/hold/commit 流程，并把复习状态回写到卡片文件里。
...
2. 若 `due_now > 0`，执行 `next --all-due`
3. 小 Claw 在聊天里只发 1 题
4. 用户作答后，先判对错，再 `hold`
5. 答对可直接 `commit`；答错则讲清楚后等“下一题/继续”再 `commit`
