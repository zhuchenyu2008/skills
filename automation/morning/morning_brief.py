...
def mmx_tts_to_mp3(text: str, voice: str, out_mp3: str, timeout=120):
    """Synthesize text to MP3 using MiniMax mmx-cli."""
    # Write text to a temp file to avoid shell-escaping issues
    txt_file = out_mp3 + '.txt'
    with open(txt_file, 'w', encoding='utf-8') as f:
        f.write(text)
    try:
        cmd = ['npx', 'mmx-cli', 'speech', 'synthesize',
               '--text-file', txt_file,
               '--voice', voice,
               '--out', out_mp3, '--quiet']
        p = subprocess.run(
        ...
        if p.returncode != 0:
            raise RuntimeError(f"mmx TTS failed: {p.stderr.strip()}")
    finally:
...
def mix_voice_with_bgm(voice_mp3: str, bgm_files: list, volume: float, fade_out_sec: float, out_ogg: str, timeout=120):
    ...
    if p.returncode != 0:
        print(f'[mix_voice_with_bgm] ffmpeg failed: {p.stderr.strip()}; falling back to voice-only', file=sys.stderr)
        wav_to_ogg_opus(voice_mp3, out_ogg)


def telegram_send_message(token: str, chat_id: int, thread_id: int, text: str, timeout=30, retries=3, backoff=2.0):
    params = {
        'chat_id': str(chat_id),
        'text': text,
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f'[telegram_send_message] attempt {attempt}/{retries} failed: {e}, retry in {sleep_s:.1f}s', file=sys.stderr)
            time.sleep(sleep_s)
    raise last_exc
...
def telegram_send_voice(token: str, chat_id: int, thread_id: int, ogg_path: str, caption: str = None, timeout=60):
    ...
    for attempt in range(1, _retries + 1):
        try:
            ...
            last_exc = e
            if attempt >= _retries or not _is_transient_error(e):
                raise
            sleep_s = _backoff * (2 ** (attempt - 1)) + random.uniform(0, 0.5)
            print(f'[telegram_send_voice] attempt {attempt}/{_retries} failed: {e}, retry in {sleep_s:.1f}s', file=sys.stderr)
            time.sleep(sleep_s)
    raise last_exc
...
def build_greeting():
    tz_now = now_in_tz('Asia/Shanghai')
    yday = (tz_now - timedelta(days=1)).strftime('%Y-%m-%d')
    mem_path = f"/root/.openclaw/workspace/memory/{yday}.md"
    mem_text = ''
    try:
    ...
    lt_path = '/root/.openclaw/workspace/MEMORY.md'
    lt_text = ''
    try:
...
只输出问候正文，不要标题，不要列表。"""
    ...
    gr_raw = sh([
        'openclaw', 'agent', '--agent', 'morning', '--session-id', daily_session_id('morning-greeting'),
...
def draft_brief(weather_consensus, rss_body, limits=None):
    limits = limits or {}
    ...
    prompt = f"""你是《早上好》topic的早报Claw。请把下面两段材料整理成适合TTS的中文口播稿。
...
"""
    ...
    raw = sh([
        'openclaw', 'agent', '--agent', 'morning', '--session-id', daily_session_id('morning-brief'),
...
def main():
    ...
    text = draft_brief(weather_consensus, rss_body, limits)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        voice = tts.get('voice') or tts.get('speaker') or 'male-qn-jingying'

        # Generate all TTS chunks concurrently to stay under RPM limit
        def gen_chunk(args):
            i, ch = args
            mp3 = os.path.join(td, f'chunk_{i:03d}.mp3')
            for attempt in range(1, 4):
                try:
                    mmx_tts_to_mp3(ch, voice, mp3)
                    return i, mp3, None
                except RuntimeError as e:
                    if 'usage limit' not in str(e):
                        return i, None, str(e)
                    if attempt < 3:
                        wait = 5 * (2 ** (attempt - 1))
                        print(f'[tts] chunk {i+1} rate-limit, retry {attempt}/3 in {wait}s...', file=sys.stderr)
                        time.sleep(wait)
            return i, None, f'tts chunk {i+1} exhausted after 3 attempts'

        mp3_map = {}
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(chunks)) as ex:
            futs = {ex.submit(gen_chunk, (i, ch)): i for i, ch in enumerate(chunks)}
            for fut in concurrent.futures.as_completed(futs):
                i, mp3, err = fut.result()
                if err:
                    raise RuntimeError(err)
                mp3_map[i] = mp3
        mp3s = [mp3_map[i] for i in sorted(mp3_map)]
        ...
        if len(mp3s) == 1:
            ...
            with open(lst, 'w', encoding='utf-8') as f:
                for mp3 in mp3s:
                    f.write(f"file '{mp3}'\n")
            voice_mp3 = os.path.join(td, 'joined.mp3')
